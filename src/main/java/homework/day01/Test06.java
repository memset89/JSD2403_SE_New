...
public class Test06 {
	public static void main(String[] args) {
		...
		//整一个StringBuilder  (方便对字符串进行修改)
		for (int i=0;i<4;i++){
			int x=random.nextInt(characters.length());
			line.append(characters.charAt(x));
			//随机一个下标 然后把下标对应的字符放进line中
		}
		System.out.println(line+"请输入验证码（不区分大小写）");
		Scanner scanner=new Scanner(System.in);
		String code=scanner.nextLine();
		if (code.equalsIgnoreCase(line.toString())){ //这玩意能直接无视大小写来判断两个字符是否相等  line需要调用toString来转成String才能被equal调用
